...
public:
     ...
     */
    virtual Eigen::VectorXf f(const Eigen::VectorXf& x, const Eigen::VectorXf& u, float t) const = 0;

    /**
     * Nonlinear Observation Function h(x, t)
     * y_k = h(x_k, t_k) + v_k
     */
    virtual Eigen::VectorXf h(const Eigen::VectorXf& x, float t) const = 0;

    /**
     * Jacobian of f w.r.t x, evaluated at (x, u, t).
     * F_k = df/dx | x_{k-1}, u_k, t_k
     */
    virtual Eigen::MatrixXf F(const Eigen::VectorXf& x, const Eigen::VectorXf& u, float t) const = 0;

    /**
     * Jacobian of h w.r.t x, evaluated at (x, t).
     * H_k = dh/dx | x_k, t_k
     */
    virtual Eigen::MatrixXf H(const Eigen::VectorXf& x, float t) const = 0;

    /**
     * Process Noise Covariance Matrix Q(t)
     */
    virtual Eigen::MatrixXf Q(float t) const = 0;

    /**
     * Measurement Noise Covariance Matrix R(t)
     */
    virtual Eigen::MatrixXf R(float t) const = 0;
